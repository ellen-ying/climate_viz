...
t_data %>% 
  ...
  geom_vline(xintercept = c(-5, -4, 0, 1), color = "green") +
  geom_segment(size = 0.75, lineend = "round") +
  geom_label(data = grid_labels, aes(x = x, y = y, label = labels), 
             inherit.aes = FALSE, fill = "black", color = "green", 
             label.size = 0, size = 3) +
  ...
  scale_y_continuous(limits = c(NA, 2035), expand = c(0,0)) +
...
ggsave("figures/climate_tornado.png", width = 4.5, height = 3.5)  
